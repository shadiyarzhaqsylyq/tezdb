...
void deserialize_row(const void* source, DynamicRow* destination, const Schema* schema) {
    ...
    for (uint32_t i = 0; i < schema->num_columns; ++i) {
        ...
        if (col->type == DATA_TYPE_INT) {
			...
			// Bound copy length to prevent buffer overflow from corrupted/malicious schemas
			
			uint32_t max_buf = sizeof(destination->values[i].str_val);
			uint32_t bytes_to_copy = (col->size < max_buf) ? col->size : max_buf;

			if(bytes_to_copy > 0) {
			memcpy(destination->values[i].str_val, src_ptr, bytes_to_copy);
			destination->values[i].str_val[bytes_to_copy - 1] = '\0';

			} else {
			destination->values[i].str_val[0] = '\0';
			}
...
typedef struct Table {
	...
	uint32_t tx_backup_root_page_num;
	Schema tx_backup_schema;
} Table;
...
// Trims leading spaces, strips trailing ';', and checks if line should be skipped
bool preprocess_input(char** line_ptr) {
    ...
    if (*line == '\0' || (line[0] == '-' && line[1] == '-')) {
        return false; // Tells runner to skip this line
    }

    // Strip trailing semicolon (;) and trailing spaces
...
void run_stream(FILE* stream, Table* table) {
    ...
    while (true) {
        ...
        if (!fgets(input_buffer, sizeof(input_buffer), stream)) {
            break; // EOF reached
        }
        ...
        execute_line(input_buffer, table);
    }
}
