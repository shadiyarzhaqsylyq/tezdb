...
void handle_leaf_underflow(Table* table, uint32_t leaf_page_num) {
    ...
    // =========================================================================
    // Try Left Sibling: Borrow or Merge
    // =========================================================================
    uint32_t left_sibling_page = INVALID_PAGE_NUM;
    if (is_right_child && num_keys > 0) {
    ...
    if (left_sibling_page != INVALID_PAGE_NUM) {
        ...
        if (left_cells > min_cells) {
            ...
            // Update routing key of left sibling in parent
        ...
        // 2. Merge Leaf into Left Sibling
        if (left_cells + *leaf_node_num_cells(leaf) <= leaf_node_max_cells(row_size)) {
            uint32_t leaf_cells = *leaf_node_num_cells(leaf);
            ...
            *leaf_node_next_leaf(left_sib) = *leaf_node_next_leaf(leaf);
            ...
            // Remove leaf from parent and reclaim page
    ...
    // =========================================================================
    // Try Right Sibling: Borrow or Merge
    // =========================================================================
    ...
    if (right_sibling_page != INVALID_PAGE_NUM) {
        ...
        if (*leaf_node_num_cells(leaf) + right_cells <= leaf_node_max_cells(row_size)) {
            uint32_t leaf_cells = *leaf_node_num_cells(leaf);
